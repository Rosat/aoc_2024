...
def _solve_problem_part1(data_file):
   ...
   with open(data_file, 'r') as f:
      ...
      f_str = f.read()
      muls = re.findall(r"mul\(\d{1,3}\,\d{1,3}\)",f_str) # Extracting all mul(x,y) in a list where x,y are integers and 0 <= x,y <=999
      pairs = [re.findall(r"\d{1,3}",s) for s in muls]              # Extracting all x,y pairs from previous list
...
def _solve_problem_part2_inelegant(data_file):
   ...
   with open(data_file, 'r') as f:
      ...
      f_str = f.read()
      occurrences = re.findall(r"mul\(\d{1,3}\,\d{1,3}\)|do\(\)|don't\(\)",f_str)  # Part 2 - Adding do() and don't() to the regex
      total = 0
      active = 1
...
def _solve_problem_part2_elegant(data_file):
   ...
   with open(data_file, 'r') as f:
      ...
      muls = re.findall(r"mul\(\d{1,3}\,\d{1,3}\)",f_str) # No change
      pairs = [re.findall(r"\d{1,3}",s) for s in muls]              # No change
